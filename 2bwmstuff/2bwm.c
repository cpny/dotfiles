    ...
    if (NULL == focuswin || NULL == focuswin->winitem){
        ...
        if (arg) {
            ...
            if (NULL == focuswin->winitem->next) {
                ...
                client = winlist->data;
                while((client->iconic || hidden(client)) && client->winitem->next!=NULL)
                    client = client->winitem->next->data;
            } else {
                client = focuswin->winitem->next->data;
                while ((client->iconic==true || hidden(client)) && client->winitem->next!=NULL)
                    client = client->winitem->next->data;
